function addNewItem() {
  ...
  if (!time || !date || !title || !description) {
...
function addNewItem() {
  ...
  if (time && title) {
    // بررسی اینکه آیا آیتم جدید تکراری است یا نه
    const itemExists = timelineItems.some(
      (item) => item.time === time && item.title === title
    );
    timelineItems = [];
    if (itemExists) {
      alert("This item already exists in the timeline!");
    } else {
